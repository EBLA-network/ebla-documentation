...
Go to [Remix](https://remix.ethereum.org/), and upload EBLA's DPoS contract's Solidity interface.&#x20;
...
* The wallet under "Account" is the wallet you want to use. Make sure you use the wallet that contains the EBLA you wish to delegate.&#x20;
...
Next, we tell Remix where the DPoS contract is located. Go the "At Address" field towards the bottom of the screen, and enter the following address. This is where EBLA's precompiled DPoS contract is located.&#x20;
...
First, we enter the number of EBLA to be delegated into the node. Within the "Deploy & Run Transactions" section of Remix, navigate back towards the top and you'll see a "Value" field. This field defines how many tokens will be transferred to the contract in this transaction.&#x20;
...
Now it has 2000 EBLA delegation (once again, reminder there are 18 extra zeros after the number), which is exactly 1000 EBLA more than before we started.&#x20;
...
In our example, since the validator only has 2000 EBLA delegation, it isn't eligible to participate in consensus yet so the reward claimed was 0, which is expected.&#x20;
